...
https://github.com/knolleary/pubsubclient
  - DoubleResetDetector by Khai Hoang
https://github.com/khoih-prog/ESP_DoubleResetDetector
  - ModbusMaster        by Doc Walker https://github.com/4-20ma/ModbusMaster
  - ArduinoJson         by Benoit Blanchon
https://github.com/bblanchon/ArduinoJson

To install the used libraries, use the embedded library manager (Sketch ->
Include Library -> Manage Libraries), or download them from github (Sketch ->
...
void SetupMqttWifiManagerMenu(MqttConfig& mqttConfig) {
  ...
  custom_mqtt_server = new WiFiManagerParameter(
      "server", "mqtt server", mqttConfig.mqttserver.c_str(), 40);
  custom_mqtt_port = new WiFiManagerParameter("port", "mqtt port",
                                              mqttConfig.mqttport.c_str(), 6);
  custom_mqtt_topic = new WiFiManagerParameter(
      "topic", "mqtt topic", mqttConfig.mqtttopic.c_str(), 64);
  custom_mqtt_user = new WiFiManagerParameter("username", "mqtt username",
                                              mqttConfig.mqttuser.c_str(), 40);
  custom_mqtt_pwd = new WiFiManagerParameter("password", "mqtt password",
                                             mqttConfig.mqttpwd.c_str(), 40);
...
void StartConfigAccessPoint(void) {
  String Text;
  Text =
      "Configuration access point started ...\r\nConnect to Wifi: "
      "\"GrowattConfig\" with your password (default: \"growsolar\") and visit "
      "192.168.4.1\r\nThe Stick will automatically go back to normal operation "
      "after " +
      String(CONFIG_PORTAL_MAX_TIME_SECONDS) + " seconds";
  ...
  if (!httpServer.hasArg("reg") || !httpServer.hasArg("val")) {
    ...
    if (httpServer.arg("operation") == "R") {
      if (httpServer.arg("registerType") == "I") {
        if (httpServer.arg("type") == "16b") {
          if (Inverter.ReadInputReg(httpServer.arg("reg").toInt(), &u16Tmp)) {
            sprintf(msg, "Read 16b Input register %ld with value %d",
                    httpServer.arg("reg").toInt(), u16Tmp);
          } else {
            sprintf(msg,
                    "Read 16b Input register %ld impossible - not connected?",
                    httpServer.arg("reg").toInt());
          }
        } else {
          if (Inverter.ReadInputReg(httpServer.arg("reg").toInt(), &u32Tmp)) {
            sprintf(msg, "Read 32b Input register %ld with value %d",
                    httpServer.arg("reg").toInt(), u32Tmp);
          } else {
            sprintf(msg,
                    "Read 32b Input register %ld impossible - not connected?",
                    httpServer.arg("reg").toInt());
          }
        }
      } else {
        if (httpServer.arg("type") == "16b") {
          if (Inverter.ReadHoldingReg(httpServer.arg("reg").toInt(), &u16Tmp)) {
            sprintf(msg, "Read 16b Holding register %ld with value %d",
                    httpServer.arg("reg").toInt(), u16Tmp);
          } else {
            sprintf(msg,
                    "Read 16b Holding register %ld impossible - not connected?",
                    httpServer.arg("reg").toInt());
          }
        } else {
          if (Inverter.ReadHoldingReg(httpServer.arg("reg").toInt(), &u32Tmp)) {
            sprintf(msg, "Read 32b Holding register %ld with value %d",
                    httpServer.arg("reg").toInt(), u32Tmp);
          } else {
            sprintf(msg,
                    "Read 32b Holding register %ld impossible - not connected?",
                    httpServer.arg("reg").toInt());
          }
        }
      }
      ...
      if (httpServer.arg("registerType") == "H") {
        if (httpServer.arg("type") == "16b") {
          if (Inverter.WriteHoldingReg(httpServer.arg("reg").toInt(),
                                       httpServer.arg("val").toInt())) {
            sprintf(msg, "Wrote Holding Register %ld to a value of %ld!",
                    httpServer.arg("reg").toInt(),
                    httpServer.arg("val").toInt());
          } else {
            sprintf(msg,
...
void loop() {
  ...
  if ((now - RefreshTimer) > REFRESH_TIMER) {
    ...
    if (pinger.Ping(GATEWAY_IP) == false) WiFi.disconnect();
#endif
